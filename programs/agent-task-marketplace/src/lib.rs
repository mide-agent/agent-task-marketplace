...
// Account structs and handlers inline for Anchor 0.32+ compatibility
...
pub mod agent_task_marketplace {
    ...
    pub fn post_task(
        ...
        require!(!title.is_empty(), MarketplaceError::EmptyTitle);
        require!(title.len() <= Task::MAX_TITLE_LEN, MarketplaceError::TitleTooLong);
        require!(description.len() <= Task::MAX_DESC_LEN, MarketplaceError::DescriptionTooLong);
        require!(!milestones.is_empty(), MarketplaceError::NoMilestones);
        require!(milestones.len() <= Task::MAX_MILESTONES, MarketplaceError::TooManyMilestones);
        
        let total: u64 = milestones.iter().map(|m| m.amount).sum();
        require!(total == budget, MarketplaceError::MilestoneAmountMismatch);
        
        let clock = Clock::get()?;
        require!(deadline > clock.unix_timestamp, MarketplaceError::InvalidDeadline);
    ...
    pub fn update_task(
        ...
        if let Some(desc) = description {
            require!(desc.len() <= Task::MAX_DESC_LEN, MarketplaceError::DescriptionTooLong);
        ...
        if let Some(dl) = deadline {
            require!(dl > clock.unix_timestamp, MarketplaceError::InvalidDeadline);
            task.deadline = dl;
        }
    ...
    pub fn submit_bid(
        ...
        require!(proposal.len() <= Bid::MAX_PROPOSAL_LEN, MarketplaceError::ProposalTooLong);
        require!(amount > 0, MarketplaceError::InvalidAmount);
        require!(timeline > 0, MarketplaceError::InvalidTimeline);
        
        let clock = Clock::get()?;
        let task = &ctx.accounts.task;
        require!(clock.unix_timestamp + timeline <= task.deadline, MarketplaceError::TimelineExceedsDeadline);
    ...
    pub fn complete_milestone(ctx: Context<CompleteMilestone>, milestone_index: u8) -> Result<()> {
        ...
        require!(index < task.milestones.len(), MarketplaceError::InvalidMilestoneIndex);
        
        let milestone = &mut task.milestones[index];
        require!(!milestone.completed, MarketplaceError::InvalidMilestoneIndex);
    ...
    pub fn release_payment(ctx: Context<ReleasePayment>, milestone_index: u8) -> Result<()> {
        ...
        let milestone = &mut task.milestones[index];
        require!(milestone.completed, MarketplaceError::MilestoneNotCompleted);
        require!(!milestone.paid, MarketplaceError::MilestoneAlreadyPaid);
    ...
    pub fn request_refund(ctx: Context<RequestRefund>) -> Result<()> {
        ...
        require!(
            ...
            MarketplaceError::RefundNotAllowed
        );
        
        let refund_amount = escrow.total_amount - escrow.released_amount;
        require!(refund_amount > 0, MarketplaceError::NoFundsToRefund);
    ...
    pub fn initialize_agent_profile(ctx: Context<InitializeAgentProfile>, name: String) -> Result<()> {
        require!(name.len() <= AgentProfile::MAX_NAME_LEN, MarketplaceError::NameTooLong);
    ...
    pub fn submit_review(
        ...
        require!(review_text.len() <= Review::MAX_REVIEW_LEN, MarketplaceError::ReviewTooLong);
